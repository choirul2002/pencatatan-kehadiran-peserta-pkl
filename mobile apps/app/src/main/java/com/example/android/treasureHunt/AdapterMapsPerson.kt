...
class AdapterMapsPerson(val dataForum:List<HashMap<String,String>>, val homePembimbingActivity: HomePembimbingActivity, val fragmentMapsPembimbing: FragmentMapsPembimbing) : RecyclerView.Adapter<AdapterMapsPerson.HolderDataForum>(){
    private var filteredList: List<HashMap<String, String>> = dataForum

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AdapterMapsPerson.HolderDataForum {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.row_maps_person,parent,false)
    ...
    override fun onBindViewHolder(holder: AdapterMapsPerson.HolderDataForum, position: Int) {
        val data = filteredList.get(position)
        holder.nama.setText(data.get("nama").toString())

        holder.card.setOnClickListener {
            fragmentMapsPembimbing.flPerson.visibility = View.GONE
            ...
            fragmentMapsPembimbing.kondisi = "person"
            fragmentMapsPembimbing.dataPerson = data.get("kode").toString()
